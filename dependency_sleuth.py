...
def get_packages_by_user(user):
    ''' Takes a single PyPI user (e.g. a package maintainer) and returns a list
        of all PyPI package-versions owned by that user. '''
...
def print_package_list_to_file(user, destination):
    ...
    package_list, dependencies = get_packages_by_user(user)
...
def get_package_versions(package):
    ...
    versions.append(release for release in packaage_json["releases"])
    print(versions)
